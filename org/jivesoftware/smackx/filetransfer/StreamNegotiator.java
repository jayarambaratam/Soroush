...
public abstract class StreamNegotiator {
    ...
    class C27401 implements Callback<NotConnectedException> {
        final /* synthetic */ XMPPConnection val$connection;
        final /* synthetic */ StreamInitiation val$response;

        C27401(XMPPConnection xMPPConnection, StreamInitiation streamInitiation) {
    ...
    protected final IQ initiateIncomingStream(XMPPConnection xMPPConnection, StreamInitiation streamInitiation) {
        ...
        try {
            IQ iq = (IQ) initationSetEvents.performActionAndWaitForEvent(streamInitiation.getFrom().toString() + '\t' + streamInitiation.getSessionID(), xMPPConnection.getPacketReplyTimeout(), new C27401(xMPPConnection, createInitiationAccept));
            if (iq == null) {
