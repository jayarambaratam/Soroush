...
public class SASLDigestMD5Mechanism extends SASLMechanism {
    ...
    /* synthetic */ class C26841 {
        static final /* synthetic */ int[] f8408xa1fe489b;

        static {
            f8408xa1fe489b = new int[State.values().length];
            try {
                f8408xa1fe489b[State.INITIAL.ordinal()] = 1;
            } catch (NoSuchFieldError e) {
            }
            try {
                f8408xa1fe489b[State.RESPONSE_SENT.ordinal()] = 2;
    ...
    protected byte[] evaluateChallenge(byte[] bArr) {
        ...
        switch (C26841.f8408xa1fe489b[this.state.ordinal()]) {
            case VideoSize.CIF /*1*/:
                ...
                while (i < length) {
                    ...
                    if ($assertionsDisabled || split2.length == 2) {
                        String replaceFirst = split2[0].replaceFirst("^\\s+", BuildConfig.FLAVOR);
                        String str = split2[1];
                        if ("nonce".equals(replaceFirst)) {
                            if (this.nonce != null) {
                                throw new SmackException("Nonce value present multiple times");
                            }
                            this.nonce = str.replace("\"", BuildConfig.FLAVOR);
